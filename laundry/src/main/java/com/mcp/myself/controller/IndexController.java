...
public class IndexController {
    ...
    public String shopping(ModelMap modelMap, HttpServletRequest request) {

        String sortProId = request.getParameter("sortProId");
        String mainProId = request.getParameter("mainProId");
        String url = "?status=0&orderBy=-1&sortStr=jump";
        if (!"".equals(sortProId) && sortProId != null) {
            url+="&sortProId="+sortProId;
        }
        if (!"".equals(mainProId) && mainProId != null) {
            url+="&mainProId="+mainProId;
        }
        modelMap.put("urlStr",url);
        return "shopping";
    }

    @RequestMapping("top.html")
    public String top(ModelMap modelMap, HttpServletRequest request) {
        return "top";
    }

    @RequestMapping("left.html")
    public String left(ModelMap modelMap, HttpServletRequest request) {
        modelMap = indexService.getIndexMainPro(modelMap);
        String sortProId = request.getParameter("sortProId");
        String mainProId = request.getParameter("mainProId");
        String id = null;
