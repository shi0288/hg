...
function toAmount() {
    ...
    if (name) {
        window.parent.location.href = 'acount.html?name=' + name;
    } else {
        ...
        success: function (rst) {
            if (rst.result) {
                ...
                $.each(list, function (key, val) {
                    ...
                    var onclickStr = "window.location.href='proDetail.html?proId=" + obj._id.$oid + "';"
...
function commitOrder() {
    ...
    if (name) {
        ...
        var payType = $("input[name='radio-choice-1']:checked").val();
        $.ajax({
            type: "POST",
...
function toConform() {
    ...
    order = order.join(";");
    localStorage.setItem("order", order.toString());
    window.location.href = 'conform.html?showwxpaytitle=1';
}
...
function ProMoveBox(objRu, str, nei) {
    ...
    testProduct(proId, numbers, function (is) {
        if (is) {
            ...
            if (panduan) {
                ...
                localStorage.setItem("order", order.toString());
                if (nei == undefined) {
                    var ele = parent.frames['topSet'].document.getElementById('header-cart-num');
                    var num = parseInt(ele.innerText) + 1;
                    ele.innerHTML = num;
                } else {
                    var num = parseInt($("#header-cart-num").html()) + 1;
